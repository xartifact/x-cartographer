...
> 状态：**设计定稿，待实施（2026-09-10；2026-09-15 域划分章节被取代）**。本文档定义用户故事地图的**实体语义与迁移方案**，替代既有文档中"用户旅程（journey）/任务（task）"的旧语义。
> ⚠️ **§2「命名空间规范」的三域划分已作废**，被 `docs/design/domain-model.md`（承诺类型：约束/工作/证据）取代——实测证明按名词分域会产生垃圾桶域，并导致 `SystemModule` 无家可归、`resolveEffectiveArchitectureContext` 穿域墙。
...
本次重设计：**两图分离，概念各归其位。**（关于"归到哪个域"，本版原按名词划分，2026-09-15 已改为按承诺类型划分，见 `domain-model.md`。）

## 2. 命名空间规范（已作废，保留供追溯）

> **作废说明（2026-09-15）**：原三域划分（用户域 / 执行域 / 平台域）依据**实体名词类型**切分，实测失效，已由 `docs/design/domain-model.md` 取代。
>
> 失效实证：
> - **平台域是剩余桶**——同时容纳 Product（作用域）/ Milestone（时间）/ StatusChange（审计）/ AdrRecord（决策），四者无统一语言、无共同一致性边界
> - **`SystemModule` 无处安放**（已发生的失败）——它是唯一横跨 ADR 与 Story/Task 双方的概念，却三域都不收；后果：`affected_modules` 0/194（story）、0/536（task），§4 解析算法不存在，模块矩阵目录未建
> - **Agent 核心操作穿域墙**——`resolveEffectiveArchitectureContext(task)` 需同时读 story（用户域）、ADR（平台域）、module（无处）
>
> 下表为原规范，**仅作历史追溯，勿据此新增实体**：
...
> **现行规范**：实体按**承诺类型**归入 约束空间 / 工作空间 / 证据空间，见 `docs/design/domain-model.md` §2。
> 命名前缀约定（`user_` / `dev_`）**继续有效**，它是命名空间卫生措施，与域归属无关。

## 3. 实体模型

### 3.1 平台实体

> 域归属见 `domain-model.md` §2：`Product`/`AdrRecord` 属**约束空间**，`StatusChange` 属**证据空间**，`Milestone` 的归属待定（见该文档 §8 Q2）。本节保留原分组仅为阅读连续性。
...
> 域归属见 `domain-model.md` §2：本组实体（含 §3.2 全部）属**约束空间·意图**。
...
**UserTask 生成规则（2026-09-15 修订）**：**以正向推演为准**——新项目/新活动录入时先声明其操作步骤，再把故事挂到步骤下。存量 194 个 story 的反向归纳**只作辅助起点**，不作层的定义来源（原版"从归位 story 的操作语义归纳"被否，理由是循环论证：数据里没有该层，是因为系统从未支持正向创建）。`user_task_id` 允许可空（总有故事落不进任何步骤），但不应长期全空。见 `domain-model.md` §8 Q3 决策说明。
