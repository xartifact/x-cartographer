  ...
  return (
    <>
      ...
      <Card
        ...
        className={cn(
          'w-60 cursor-pointer transition-all duration-150 bg-background',
          'hover:shadow-lg hover:-translate-y-0.5',
          selected || isSelected ? 'ring-2 ring-primary shadow-md' : 'shadow-sm',
        ...
        <CardContent className="p-3 space-y-2">
          ...
          <div className="flex items-center justify-between gap-1">
            <div className="flex items-center gap-1.5 min-w-0">
              ...
              <span className={cn('text-[10px] font-semibold shrink-0', pl.cls)}>
                {pl.text}
              </span>
            </div>
            {story.status && (
  ...
  ({ data }) => {
    ...
    return (
      <div className="flex items-center justify-center">
        <Card className="w-60 bg-primary/5 border-primary/20">
          <CardContent className="p-4 text-center">
            <h3 className="font-semibold text-sm line-clamp-2">{journeyName}</h3>
            <p className="text-xs text-muted-foreground mt-1">{storyCount} 个故事</p>
          </CardContent>
        </Card>
      </div>
    );
