...
describe('formatProjectStats', () => {
  it('从产品树实时统计故事数，不依赖 metadata', () => {
    const stats = formatProjectStats({
      user_activities: [
        { id: 'UA-1', stories: [{ id: 'US-1' }, { id: 'US-2' }] },
        { id: 'UA-2', stories: [{ id: 'US-3' }] },
      ],
      metadata: {},
    });
  ...
  it('metadata.total_stories 缺失时故事数不为 0（回归：服务端从未写该字段）', () => {
    const stats = formatProjectStats({
      user_activities: [{ id: 'UA-1', stories: [{ id: 'US-1' }] }],
  ...
  it('无活动时全为 0', () => {
    const stats = formatProjectStats({ user_activities: [], metadata: {} });
    expect(stats).toEqual({ activityCount: 0, storyCount: 0, taskCount: 0 });
  });

  it('任务数从 stories[].dev_tasks 统计', () => {
    const stats = formatProjectStats({
      user_activities: [
        { id: 'UA-1', stories: [{ id: 'US-1', dev_tasks: [{ id: 'T-1' }, { id: 'T-2' }] }] },
      ],
      metadata: {},
    });
